...
export const getMonthlyIncome = async () => {
  try {
    ...
    const res = await instanceAxios.get(`${API_ENDPOINT.REPORT.INDEX}/order-status/monthly-income`)
...
export const getMCustomerOrders = async () => {
  try {

    const res = await instanceAxios.get(`${API_ENDPOINT.REPORT.INDEX}/order-status/customer-orders`)
