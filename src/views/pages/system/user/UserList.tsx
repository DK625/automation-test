...
const UserListPage: NextPage<TProps> = () => {
  ...
  useEffect(() => {
    if (isFirstRender.current) {
      setFilterBy({ roleId: "", status: statusSelected, cityId: citySelected, userType: typeSelected })
    }
    ...
    <>
      ...
      >
        ...
        <Grid container sx={{ height: '100%', width: '100%' }}>
            <Box>
                <Typography sx={{fontSize: "25px", fontWeight: "600", color: "blue"}}>Danh sách người dùng</Typography>
            </Box>
          {!selectedRow?.length && (
            <Box
