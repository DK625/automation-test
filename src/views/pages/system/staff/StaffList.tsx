...
import CreateEditUser from 'src/views/pages/system/staff/component/CreateEditStaff'
...
const StaffListPage: NextPage<TProps> = () => {
  // ** Translate
  const { t } = useTranslation();
  const pathname = usePathname();
  const typeApplication = pathname.split('/')[2];
  console.log("typeApplication", typeApplication);
  ...
  const handleGetListUsers = () => {
    console.log("filterBy1", filterBy, optionRoles);
    // if(filterBy.roleId.length <=0){
    //   filterBy.roleId = []
    // }
    const query = {
  ...
  useEffect(() => {
    console.log("optionRoles", optionRoles);
    const arrIdRole: any = [];
    let convertRoleSelected = roleSelected;
    if(roleSelected.length <=0){
      if(optionRoles.length > 0) {
        optionRoles.map(el => arrIdRole.push(el.value))
      }
      convertRoleSelected = arrIdRole;
    }
    if (isFirstRender.current) {
      setFilterBy({ roleId: convertRoleSelected, status: statusSelected, cityId: citySelected, userType: typeSelected })
    }
  }, [roleSelected, statusSelected, citySelected, typeSelected, optionRoles])
  ...
  return (
    <>
      ...
      >
        ...
        <Box>
                <Typography sx={{fontSize: "25px", fontWeight: "600", color: "blue"}}>Danh sách nhân viên</Typography>
            </Box>
          {!selectedRow?.length && (
            <Box
