...
import {cancelOrderProductByAdmin, getAllOrderProducts} from 'src/services/order-product'
...
export const orderProductSlice = createSlice({
    ...
    extraReducers: builder => {
        ...
        // ** delete orders product
        builder.addCase(cancelOrderProductByAdminAsync.pending, (state, action) => {
            state.isLoading = true
        })
        builder.addCase(cancelOrderProductByAdminAsync.fulfilled, (state, action) => {
            state.isLoading = false
            state.isSuccessCancel = !!action.payload?.data?._id
            state.isErrorCancel = !action.payload?.data?._id
            state.messageErrorCancel = action.payload?.message
            state.typeError = action.payload?.typeError
        })
    }
})
